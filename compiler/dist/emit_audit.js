...
exports.emitAuditMiddleware = exports.emitAuditSchema = void 0;
// ─── emitAuditSchema ─────────────────────────────────────────────────────────
function emitAuditSchema() {
...
exports.emitAuditSchema = emitAuditSchema;
// ─── emitAuditMiddleware ──────────────────────────────────────────────────────
function emitAuditMiddleware(system) {
    const lines = [];
    ...
    lines.push(`          (actor_id, action, entity_type, entity_id, payload, ip_address, user_agent)`);
    lines.push(`          VALUES ($1, $2, $3, $4, $5, $6, $7)\`,`);
    lines.push(`        [actorId, action, entityType ?? null, entityId, JSON.stringify(redacted), ipAddress, userAgent]`);
    lines.push(`      );`);
    lines.push(`    } catch (err) {`);
    lines.push(`      // Audit failures must not block the request — log and continue`);
    ...
    lines.push(`}`);
    lines.push("");
    lines.push(`export async function getAuditLog(entityType: string, entityId: string): Promise<any[]> {`);
    lines.push(`  return await query(`);
    lines.push(`    "SELECT * FROM audit_log WHERE entity_type = $1 AND entity_id = $2 ORDER BY created_at DESC LIMIT 100",`);
...
exports.emitAuditMiddleware = emitAuditMiddleware;
//# sourceMappingURL=emit_audit.js.map