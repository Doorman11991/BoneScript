...
class Lowering {
    ...
    lowerSystem(sys, sourceHash) {
        ...
        const stores = sys.declarations.filter((d) => d.kind === "StoreDecl");
        const eventDecls = sys.declarations.filter((d) => d.kind === "EventDecl");
        const constraints = sys.declarations.filter((d) => d.kind === "ConstraintDecl");
    ...
    lowerEntity(entity, capabilities, stores, policies = []) {
