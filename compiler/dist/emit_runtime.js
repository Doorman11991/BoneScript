...
exports.emitMigration = exports.emitIndex = exports.emitStateMachineRuntime = exports.emitEntityRouter = exports.emitAuthMiddleware = exports.emitEventBus = exports.emitDbClient = exports.emitTsConfig = exports.emitPackageJson = void 0;
...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || "postgresql://localhost:5432/${toSnakeCase(system.name)}",
  max: 20,
});
...
// ─── Event Bus stub (real impl in emit_events.ts) ─────────────────────────────
function emitEventBus() {
    return `// Generated by BoneScript compiler. DO NOT EDIT.
// NOTE: This stub is replaced by the durable event bus from emit_events.ts
export const eventBus = {
  subscribe(_type: string, _handler: any) {},
  async publish(_type: string, _payload: any, _source: string) {},
  startWorker(_interval?: number): null { return null; },
};
`;
}
exports.emitEventBus = emitEventBus;
// ─── Auth Middleware ──────────────────────────────────────────────────────────
...
function emitEntityRouter(mod, system) {
    ...
    // Build JOIN clause for has_one and belongs_to relations
    ...
    if (joinRelations.length > 0) {
        const joinClauses = joinRelations.map(r => {
            const alias = r.to_table.slice(0, -1); // remove trailing 's'
    ...
    if (mod.state_machines.length > 0) {
        ...
        lines.push(`    const tr = transition${sm.entity}(current.state as any, fields.state);`);
        lines.push(`    if (!tr.ok) return res.status(422).json({ error: { code: "INVALID_TRANSITION", message: tr.error } });`);
        lines.push(`  }`);
    }
