...
function deadModuleElimination(s, log) {
...
function storeMerging(s, log) {
    ...
    for (let i = 0; i < stores.length; i++) {
        for (let j = i + 1; j < stores.length; j++) {
            ...
            if (a.config["engine"] === b.config["engine"] &&
                a.models.length === 0 &&
                b.models.length === 0 &&
                !toRemove.has(a.id)) {
                toRemove.add(b.id);
...
function eventDeduplication(s, log) {
    const seen = new Set();
    const events = [];
...
function dependencyMinimization(s, log) {
...
function indexOptimization(s, log) {
    const modules = s.modules.map(m => {
        const models = m.models.map(model => {
            ...
            const indexes = model.indexes.filter((idx, i) => {
                const isPrefix = model.indexes.some((other, j) => {
                    if (i === j)
                        return false;
                    if (other.fields.length <= idx.fields.length)
                        return false;
