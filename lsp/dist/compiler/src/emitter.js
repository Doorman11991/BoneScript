...
function toTsType(irType) {
    ...
    if (setMatch)
        return `Set<${toTsType(setMatch[1])}>`;
    const mapMatch = irType.match(/^map<(.+),\s*(.+)>$/);
    if (mapMatch)
        return `Map<${toTsType(mapMatch[1])}, ${toTsType(mapMatch[2])}>`;
...
function toSqlType(irType) {
    if (SQL_TYPE_MAP[irType])
        return SQL_TYPE_MAP[irType];
    if (irType.startsWith("list<") || irType.startsWith("set<") || irType.startsWith("map<"))
        return "JSONB";
    if (irType.startsWith("optional<"))
        return toSqlType(irType.slice(9, -1));
    return "JSONB";
}
...
class Emitter {
    ...
    emitSchema(model, mod, system) {
        ...
        for (const field of model.fields) {
            ...
            if (field.default_value) {
                if (field.default_value === "gen_random_uuid()")
    ...
    emitStateMachine(sm, mod, system) {
        const lines = [];
        lines.push(`// Generated by BoneScript compiler. DO NOT EDIT.`);
        lines.push(`// Source: ${system.source_hash}`);
        ...
        for (const mod of system.modules) {
            if (mod.kind === "api_service" || mod.kind === "realtime_service") {
                ...
                for (const dep of mod.dependencies) {
                    ...
                    if (depMod)
                        lines.push(`      - ${toSnakeCase(depMod.name)}`);
                }
    ...
    emitInfraConfig(system) {
        ...
        for (const mod of system.modules) {
            if (mod.kind === "data_store") {
                ...
                if (mod.config["retention_ms"])
                    lines.push(`      retention_ms: ${mod.config["retention_ms"]}`);
                if (mod.config["partition_key"])
                    lines.push(`      partition_key: ${mod.config["partition_key"]}`);
                lines.push(``);
            }
        }
        // Gateway
